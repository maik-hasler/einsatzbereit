...
public sealed record GetVolunteerOpportunityDateAvailabilityRequest(
	...
	// (frontend/src/client/generated/, IntegrationTests/ApiClient.cs) stay
	// wire-compatible until their next regeneration, which can drop this
	// property. The frontend half has since regenerated (@hey-api, ADR-10),
	// so dropping it now costs only the C# client's next rebuild. The caller's zone comes from the X-Timezone header instead
