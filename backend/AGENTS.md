...
- **Creating organizations in a test: always navigate to them by id** (`/app/{organizationId}/...`), never via `GoToOrgAppDashboardAsync`/the "Organization overview" CTA. The test accounts are shared across the whole session, and `activeOrg.ts`'s `resolveActiveOrg` falls back to "first organization alphabetically by name" when no `active-org` cookie is set. A throwaway org whose name sorts before the seeded `Fairview ...` orgs therefore hijacks the CTA for every *other* concurrently-running test on that account - the cause of a long-running class of "sometimes green, sometimes red" failures (a test would land on some unrelated test's org and fail on missing content). `FastSignInAsync` now pins the `active-org` cookie to the seeded org (see `AspireFixture.GetSeededOrganizerOrganizationIdAsync`) so the CTA is deterministic regardless of what else the suite created - but that only protects the *consumer* side, so keep creating-and-navigating explicit by id
