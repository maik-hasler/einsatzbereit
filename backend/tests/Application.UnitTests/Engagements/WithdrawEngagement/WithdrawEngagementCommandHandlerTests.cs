...
public class WithdrawEngagementCommandHandlerTests
{
		...
		CancellationToken cancellationToken)
	{
		...
		await _notifRepo.Received(1).AddAsync(
			Arg.Is<Notification>(n => n!.RecipientId == UserId.Create(organizerUserId).GetValueOrThrow()
	...
	[Test]
	public async Task Handle_ShouldRaiseWithdrawnEvent_ForThePostCommitEmailHandler(
		CancellationToken cancellationToken)
	{
		// #1150: the organizers' email moved to EngagementWithdrawnNotificationHandler,
		// dispatched post-commit.
		var (engagement, volunteerId) = CreatePendingEngagementWithVolunteer();
		var engagementId = EngagementId.New();
		_engagementRepo.FindAsync(engagementId, cancellationToken).Returns(engagement);

		await _sut.Handle(new WithdrawEngagementCommand(engagementId, volunteerId.Value), cancellationToken);

		engagement.Events.Should().ContainSingle(e => e is EngagementWithdrawnDomainEvent);
	}
}
