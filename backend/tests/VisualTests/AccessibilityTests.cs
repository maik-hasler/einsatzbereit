...
public class AccessibilityTests(AspireFixture fixture) : VisualTestBase(fixture)
{
	...
	{
		// Vera's shared seed engagements are never checked in, so navigating
		// straight there would only ever scan the empty-state fallback, not the
		// populated table this issue actually introduced. Seed a fresh
		// org/opportunity/slot/engagement and drive it through confirm +
		// check-in, mirroring EngagementManagementPage_AsOlaf below, so this
		// deterministically covers the table.
		var frontend = Fixture.GetEndpoint("frontend");
		var backend = Fixture.GetEndpoint("backend");
		...
		var orgResponse = await olafHttp.PostAsJsonAsync(
			"/v1/organizations", new { name = $"EngagementRecordA11y Org {suffix}" });
		orgResponse.EnsureSuccessStatusCode();
		var org = await orgResponse.Content.ReadFromJsonAsync<JsonElement>();
		var organizationId = org.GetProperty("id").GetProperty("value").GetString();
		...
		{
			title = $"EngagementRecordA11y Opportunity {suffix}",
		...
		// Confirm this actually scanned the populated table, not the empty state.
