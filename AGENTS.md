...
3. Create the release branch **from the feature branch** (so the fix is included):
   ...
   ```
4. `release-rc.yml` creates the tag; `publish.yml` builds images, pushes them to GHCR, and fires a `repository_dispatch` event at `maik-hasler/mgmt-hetzner` to trigger the actual deploy (this repo's own workflow stops at publishing - see `.github/AGENTS.md`). Monitor `publish.yml` via `mcp__github__pull_request_read get_check_runs` on the release commit, then monitor mgmt-hetzner's `deploy-einsatzbereit.yaml` run the same way (or poll each repo's Actions tab) - deploy-staging succeeding means that second, separate-repo run reported success, not just that `publish.yml` went green.
