...
- **CI's `editorconfig` job cannot be run via `npx` here** - `npx editorconfig-checker` resolves the binary through the GitHub releases API, which the sandbox's git proxy answers with 403. Fetch the release asset directly instead (`curl -sSL https://github.com/editorconfig-checker/editorconfig-checker/releases/download/v3.11.2/ec-linux-amd64.tar.gz | tar xz`, then run `bin/ec-linux-amd64 -config .editorconfig-checker.json`). Worth doing whenever a change adds a `.py`, `.adoc` or `.puml` file: nothing else in the local toolchain checks indentation, so the first signal is otherwise a red PR.
