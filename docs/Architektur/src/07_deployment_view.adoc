...
=== Infrastructure Level 1: Local Development (Docker Compose)
...
Motivation::
The entire stack runs locally in Docker using a single `docker compose up --build` command. This ensures environment parity across all contributor machines and eliminates manual setup of databases, Keycloak, or service dependencies.
...
* Frontend: `npx playwright test` (E2E tests against the full Docker Compose stack)
