...
*Self-hosted over SaaS.* No third-party observability or alerting vendor is wired in by default - not Datadog, not Grafana Cloud, not an incident platform. The deployed host runs its own Grafana, Prometheus, Loki, Tempo and Alertmanager instead - owned by the separate `mgmt-hetzner` repository as shared, label-driven infra (maik-hasler/mgmt-hetzner#2, ADR-7) rather than by this repo's `docker-compose.yml`. The trade-off is deliberate: it keeps operational complexity and cost inside one host's compose files and avoids a data-sharing relationship with a vendor for a platform handling volunteer contact data. Any OTLP backend can be pointed at the exporter if that changes.

*Exposure of the monitoring stack.* Prometheus, Alertmanager, node-exporter, Loki and Tempo sit on an internal-only `monitoring` Docker network (external to this repo, owned by `mgmt-hetzner`) with no reverse-proxy route, because none of them has built-in authentication and each would leak metric, log, trace or alert content if reachable from the internet. Grafana is the only one with a public route, gated by its own admin login.

*Opt-in, label-driven discovery.* Any container on the host - from either repo's compose file - opts into metrics and logs the same way it already opts into Traefik routing: by setting labels. `prometheus.scrape=true` plus `prometheus.port=<port>` gets a container scraped by Prometheus's `docker_sd_configs` once it also joins the `monitoring` network; `logs.scrape=true` gets its stdout shipped to Loki by Alloy, independent of network membership. This repo's backend sets both; every other container in this repo's `docker-compose.yml` sets `logs.scrape=true` only, matching what Alloy already shipped before the move (every container this repo defines, not just the backend).

*Log and trace collection.* Grafana Alloy ships opted-in containers' logs to Loki, discovering containers through a read-only mount of the host's Docker socket. This is the standard pattern for Docker-aware log shippers and a narrower grant than cAdvisor's (list containers and read their logs, rather than full host filesystem and cgroup access, which is why cAdvisor was left out), but a compromised container with socket access still has effective host-level reach - it is not risk-free. Per-container metrics remain a possible follow-up if the host-level view proves insufficient. Tempo needs no shipper: the backend pushes traces to it directly over OTLP, since the application already speaks OTLP natively.

*Alerting.* Prometheus evaluates alert rules and forwards firing alerts to Alertmanager, which emails them over an SMTP relay - a separate copy of the same mailbox credentials the backend and Keycloak use for their own mail (#1081), held in `mgmt-hetzner`'s own secrets since Alertmanager moved there. The only rule shipped so far is `InstanceDown` (any scrape target unreachable for two minutes) - deliberately the one signal that needs no application-specific metric knowledge, and generic enough that it now lives in `mgmt-hetzner`'s own base rule set rather than as an einsatzbereit-specific contribution. Prometheus also globs a `rules.d` directory any app repo can drop its own rules into on release, without a PR against `mgmt-hetzner` - not yet used by this repo, since it has no rule of its own beyond the shared `InstanceDown` one.
