...
Quality and Performance Characteristics::
All public traffic is HTTPS via Traefik. PostgreSQL data is backed up daily with tiered retention, and the MinIO bucket is backed up daily with a flat retention window (#1087) - both backups still live on the same host as the data they protect, since staging is a single server with no off-host target to replicate to; this covers accidental deletion/corruption but not loss of the host itself. Container images are the release artifacts pulled from GHCR; the backend applies EF Core migrations on startup (`Database:MigrateOnStartup`).
...
|`minio/mc`
|internal
|Daily mirror of the einsatzbereit bucket, retention 7 days
