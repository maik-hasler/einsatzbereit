...
This chapter describes the infrastructure and which building blocks run where. The operational decisions behind that infrastructure - what the backup strategy guards against - are cross-cutting and live in <<section-concepts>> under Operations.
...
Release candidate tags (`v{semver}-rc.N`) publish images without moving the `latest` tag; stable tags (no `-rc` suffix) publish images and move `latest`. Either way, `publish.yml` stops there - it does not deploy, and does not trigger anything to deploy. Deploying a published tag is a deliberate, separate, manual step against a different, private infrastructure repository (`mgmt-hetzner`), which owns `docker-compose.yml` and the actual deploy for this host (see "Staging and Production" below). See link:https://github.com/maik-hasler/einsatzbereit/blob/main/VERSIONING.md[VERSIONING.md] for the full versioning and publishing strategy.
...
Motivation::
The deployed environment serves real users at `*.maik-hasler.de`. It runs on a single host (a Hetzner `cx43`: 8 vCPU / 16 GB RAM / 40 GB disk), provisioned via Terraform and deployed via `docker-compose.yml` from a separate, private infrastructure repository (`mgmt-hetzner`) - this repo only builds and publishes the images that compose file references, it does not deploy them (see "Image Publishing" above). The host runs behind a shared, host-level Traefik reverse proxy that terminates TLS (certificates via ACME / Let's Encrypt), which mgmt-hetzner also owns, alongside a shared Prometheus/Grafana/Loki/Tempo monitoring stack this app's containers opt into via Docker labels. The same host also runs Blogfolio, an unrelated personal site - the app stacks share the machine but are otherwise fully independent (separate repos, separate `docker compose` projects, no shared services beyond Traefik/monitoring). This is the environment every release candidate is verified against before a change is considered done.
...
Deployment is manually triggered and split across two repositories. A version tag pushed here triggers this repo's `publish.yml`, which builds and pushes the images to GHCR and stops - it does not deploy them or notify anything to. Deploying a published tag is a separate `workflow_dispatch` run of `mgmt-hetzner`'s `deploy-einsatzbereit.yaml` (version + target environment as inputs), which does the actual deploy to the host over SSH (`docker compose pull` + `up`) via one of two jobs - staging or production (#1344), mirroring the split this repo used before the move. Both jobs deploy to the exact same host and `/opt/einsatzbereit` directory described in the table above - there is only one live host today, and it is deliberately staging/demo infrastructure rather than a hardened production box (see root `AGENTS.md`'s Test Users note). See TDR-3 for why the two-job split exists despite both jobs targeting the same host today.

=== Infrastructure Level 2: Deployed Host

The deployed host is the one infrastructure element whose internal structure matters, because network segmentation is what keeps PostgreSQL unreachable from outside this stack. Everything above runs as containers on a single Docker daemon, split across networks:
...
|`monitoring` (external)
|Backend API
|No Traefik route from this repo's own containers.
|===

Two containers deliberately straddle networks. The *Backend API* is on all three: `proxy` to serve public traffic, `db` to reach PostgreSQL, `monitoring` to export metrics and traces. *Keycloak* is on `proxy` and `db` for the same reason at a smaller scale.

The Backend API is therefore where an authentication gap has the widest blast radius, which is why it validates every JWT against Keycloak's JWKS. MinIO is the one supporting service with a public route of its own; it is reached over `proxy` rather than the database network, and the backend authenticates to it with a bucket-scoped service account rather than root credentials (#1353).
