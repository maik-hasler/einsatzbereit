...
|NSwag
|A .NET toolchain for generating API clients from OpenAPI specifications. Here it generates exactly one artefact: the integration tests' C# client (`ApiClient.cs`), which must never be hand-edited. It does not produce the OpenAPI document - that comes from ASP.NET's `GenerateOpenApiDocuments` - and the frontend's TypeScript client comes from `@hey-api/openapi-ts` instead.
