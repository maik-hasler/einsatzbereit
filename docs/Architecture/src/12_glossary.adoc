...
|NSwag
|A .NET toolchain for generating API clients from OpenAPI specifications. It generates the OpenAPI document itself and the integration tests' C# client (`ApiClient.cs`), both of which must never be hand-edited. The frontend's TypeScript client comes from `@hey-api/openapi-ts` instead.
