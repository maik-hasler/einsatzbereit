...
|Provides the web UI for volunteers, organizers and platform admins. Handles OIDC login redirect and token storage. Communicates with the backend API using a generated client (`@hey-api/openapi-ts`).
...
The interfaces of the four Level-1 containers are consistent with, and not repeated from, the technical context's External Interfaces table in <<section-context-and-scope>>, which remains the authoritative list. The one context interface without a Level-1 counterpart here is GitHub Actions to GHCR, which touches build artifacts rather than a running building block - see <<section-deployment-view>>.
...
|Users
|Profiles, notification preferences, streaks, account deletion, and the admin-facing user administration.
...
Modules communicate in one of two ways. Direct calls go inward through the shared kernel only for reads a use case cannot complete without (Engagements reads opportunity status and slot capacity). Everything reactive - notifications, achievements - runs off domain events raised by the aggregate and dispatched after commit, so the module that causes an event never references the modules that react to it.
...
|Components
|Reusable UI building blocks - stateless or locally stateful, styled with Tailwind CSS. Includes the domain-flavoured ones (`SignUpModal`, `CheckInModal`, `OpportunityCard`) and the generic ones (`Button`, `Modal`, `Field`).
|`frontend/src/components/`

|API Client
|TypeScript client generated from the backend's OpenAPI document by `@hey-api/openapi-ts`, providing a typed function per endpoint - see <<section-concepts>> under Development and Build.
|`frontend/src/client/generated/`, wrapped by `frontend/src/client/index.ts`
...
|Contexts and Hooks
|Global UI state (`ToastContext`, `QuickActionsContext`, `HeaderOverlayContext`, `OrgBreadcrumbContext`, `AuthStatusContext`) and reusable logic (`useApiClient`, `useAchievementNotifier`, `useLoadMore`).
...
* `backend/src/Aspire/AppHost/` is local orchestration only and is never part of a released image - see <<section-deployment-view>>.
