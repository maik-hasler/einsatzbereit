...
|The .NET Aspire AppHost provisions the local service containers (see <<section-concepts>> under Development and Build for the one-command orchestration), which requires Docker on every contributor machine. Docker Compose is the deployment mechanism (see chapter 7), not the local-development one.
