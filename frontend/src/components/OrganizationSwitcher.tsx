	...
	return (
		<>
			<div className="relative" ref={containerRef}>
				...
				{open && (
					<div
						className={`absolute left-0 top-full mt-2 w-64 rounded-lg border shadow-lg z-50 ${transparent ? "border-white/20 bg-brand-800" : "border-gray-200 bg-white"}`}
					>
						<div className="py-1 max-h-60 overflow-y-auto">
							{orgs.map((org) => (
								...
								>
									...
									{org.id === activeOrgId && (
										<svg
											className={`ml-auto w-4 h-4 ${transparent ? "text-white" : "text-brand-500"}`}
											fill="none"
											viewBox="0 0 24 24"
											strokeWidth="2"
											stroke="currentColor"
						...
						>
							{activeOrgId && (
								<>
									<button
										...
										className={`flex w-full items-center gap-3 px-4 py-2.5 text-sm transition-colors ${transparent ? "text-white/90 hover:bg-white/10 hover:text-white" : "text-gray-700 hover:bg-gray-50"}`}
									>
										<svg
							...
							<button
								...
								className={`flex w-full items-center gap-3 px-4 py-2.5 text-sm transition-colors ${transparent ? "text-white/90 hover:bg-white/10 hover:text-white" : "text-brand-700 hover:bg-brand-50"}`}
							>
								<svg
									className="w-4 h-4"
									fill="none"
