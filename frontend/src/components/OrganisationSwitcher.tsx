...
import type { KeycloakOrganisation } from "../client/api-client";
...
export default function OrganisationSwitcher({ activeOrgId }: Props) {
  const [orgs, setOrgs] = useState<KeycloakOrganisation[]>([]);
  ...
  const fetchOrgs = () => {
    setLoading(true);
    fetch("/api/organisationen")
      .then((res) => {
        if (!res.ok) throw new Error(`Fehler ${res.status}`);
        return res.json();
      })
      .then((data: KeycloakOrganisation[]) => {
        setOrgs(data);
  ...
  const handleSwitch = (org: KeycloakOrganisation) => {
