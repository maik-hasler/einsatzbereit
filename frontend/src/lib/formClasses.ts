...
// global.css, not from a low-contrast focus:ring-*/outline-none pair here -
// see Button.tsx's BASE_CLASSES for the same reasoning.

// Kept separate from inputClass's own `block`/`mt-1` layout classes so a
// flex-based trigger (e.g. Dropdown) can reuse the surface without a
// block/flex classname contradiction.
// min-h-10 (not a fixed h-10): each browser's native <select> chrome renders
// a couple pixels taller than a same-padded input, so only a floor height
// lines the two up - Button.tsx's md size is the matching half.
...
// bg-position and pr-9 keep the inline chevron clear of the text at both
// paddings.
