...
| `formClasses` | `lib/formClasses.ts`         | `inputClass`, `textareaClass`, `labelClass` for every form control, plus `checkboxClass` for every checkbox - a native checkbox takes its checked fill from `accent-color` and nothing else, so `text-brand-*` on one is inert and it paints the browser's default blue (#2329). `formClasses.test.ts` scans the sources and fails on any checkbox that does not use it |
