...
- **Baseline: 80.95 %** (897 generated / 840 scored / 680 killed / 126 survived / 34 no-coverage / 57 ignored), 36 min 24 s on a contended 4-core sandbox at `concurrency: 2`. Recorded on #2148's head. Comparable only against another StrykerJS 10.0.0 run of the same `mutate` scope - a version bump or a scope change makes the number a different number.
