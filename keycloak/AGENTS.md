...
These accounts ship `"enabled": false` in the same realm file that is baked into the published Keycloak image (#2197: an enabled `admin` account with a published password in a released image is a live takeover, no exploitation required). `enabled: false` skips Keycloak's password-policy validation during `--import-realm`, same as `frontend-test` above, so the pre-hashed short dev passwords still import cleanly. `AppHost.cs` flips all three back to `enabled: true` in the dev-only realm copy it writes before import - that's the only path that ever needs them live for local dev/demo/QA use (#1166) or for `IntegrationTests`/`VisualTests`, both of which boot Keycloak through the Aspire AppHost. Anywhere the released image runs, sign in with an operator-provisioned account instead - see `README.md`'s variable table for how to create one. Do not remove these accounts from the realm file or re-enable them in the shipped image without checking with the repo owner first.
