...
These accounts ship in the same realm file that is baked into the published Keycloak image, so anywhere that image runs they exist with the exact passwords above. That is intentional for a demo/QA environment (#1166), and it is why running this image anywhere that matters means overriding these users. Do not "fix" it by removing the accounts from the shipped image without checking with the repo owner first.
