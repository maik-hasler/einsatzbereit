...
| `.claude/` | AI tooling for Claude Code: check agents (a11y, architecture, ef-migration, i18n, nswag), skills (`field-review` itself, `self-review`, `ingest`/`query`/`lint` for `wiki/`), hooks (incl. `protect-generated-clients.sh`). Treat as first-class repo content, not junk. |
