...
- Every non-obvious claim is verifiable; cite the specific source - `sources/<path>`, a repo path (optionally `@<commit-sha>`), or `#NNN` / a full issue-or-PR URL. Never invent a reference, commit SHA, issue number, or date.
