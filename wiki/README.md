...
- `docs/` - formal, reviewed arc42 architecture documentation, ADRs, TDRs.
  AsciiDoc, built to GitHub Pages. Stays authoritative for anything that
  belongs there.
...
1. **`sources/`** - your own loose notes and ideas about Einsatzbereit,
   dropped by hand as plain Markdown (see `sources/README.md`). Raw and
...
2. **The repo itself** - commits, code, hook scripts, existing `CLAUDE.md`
   ...
   directly by repo-relative path, no copy into `sources/` needed.
...
├── sources/           Your raw, immutable notes. Read-only by convention.
...
`sources/`, `scripts/`, or `.claude/` is a concept document too.
...
whatever you drop in `sources/`, then decide via issue #701 whether it's
