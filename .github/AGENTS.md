...
- **Coverage (#1327):** each `dotnet run` test invocation passes `-- --coverage --coverage-output-format cobertura --coverage-output <Project>.cobertura.xml` - `Microsoft.Testing.Extensions.CodeCoverage` ships bundled with the `TUnit` meta package, no separate `PackageReference` needed (confirmed locally: the flag works out of the box against the existing `TUnit` version pin). `fast-tests`/`integration-tests`/`visual-tests` each upload their cobertura file(s) as a `coverage-*` artifact; a `coverage-report` job (`needs` all three, `if: github.event_name == 'pull_request'`) downloads and merges them with `irongut/CodeCoverageSummary` and posts/updates a sticky PR comment via `marocchino/sticky-pull-request-comment`. `IntegrationTests`/`VisualTests` boot the API as a separate Aspire-orchestrated process, so their coverage numbers reflect whatever that profiler-based instrumentation captures across that process boundary, not a guaranteed full picture of `Api`/`Infrastructure` - treat their percentages as a lower bound, not authoritative.
