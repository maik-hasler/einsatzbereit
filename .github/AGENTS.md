...
**Release candidates:** `-rc.N` suffix (e.g., `v1.0.0-rc.1`). Image is published, tagged `staging` instead of `latest`, and a dispatch tells mgmt-hetzner to run its `deploy-staging` job.

**Full release:** Tag without `-rc` suffix → image published + `latest` tag updated, and a dispatch tells mgmt-hetzner to run its `deploy-production` job.
...
5. Tag with version + `latest` (if not RC) or `staging` (if RC) - `publish-backend` also exposes this version string as a job output (`outputs.version`), which `trigger-deploy` below forwards to mgmt-hetzner instead of deploying it itself
6. `github-release` job (`needs: [publish-backend, publish-frontend, publish-keycloak]`, so it runs for both stable and RC tags) creates a GitHub Release for the tag via `gh release create`, using the default `GITHUB_TOKEN` (job-scoped `permissions: contents: write`, no `RELEASE_TOKEN` needed - creating a release doesn't push a ref, so it doesn't hit the "tags pushed with `GITHUB_TOKEN` don't trigger workflows" restriction that `release-rc.yml` works around). Notes are generated from commit subjects since the previous tag (found by semver sort across both stable and RC tags, via `git tag --sort=-v:refname`), grouped by Conventional Commit type - a `!` after the type/scope (e.g. `feat!:`) surfaces under a "Breaking Changes" section. See [VERSIONING.md](../VERSIONING.md)'s "Release Notes" section - GitHub Releases is the canonical record, there is no `CHANGELOG.md` file.
7. `trigger-deploy` (`needs: [publish-backend, publish-frontend, publish-keycloak]`) fires a `repository_dispatch` event (`einsatzbereit-deploy`) at `maik-hasler/mgmt-hetzner`, carrying `version` and `prerelease` as `client_payload` - see below

### Deployment moved to mgmt-hetzner

Deployment (`docker-compose.yml`, the SSH/health-gate/rollback logic, and the Postgres/Keycloak/MinIO/SMTP/offsite-backup secrets it needs) does **not** live in this repo. It moved to `maik-hasler/mgmt-hetzner`, which also owns Traefik and the shared monitoring stack on the same host - see that repo's `README.md` and `.github/workflows/deploy-einsatzbereit.yaml`. This repo's `publish.yml` stops once images are built and pushed; `trigger-deploy` above is its only involvement in what gets deployed where.

mgmt-hetzner's `deploy-einsatzbereit.yaml` mirrors the old `deploy-staging`/`deploy-production` split as two jobs/GitHub Environments (`einsatzbereit-staging`/`einsatzbereit-production`) gated on the dispatched `prerelease` flag, for the same reason that split originally existed (#1344): both currently deploy to the same single host and `/opt/einsatzbereit` directory (`einsatzbereit.maik-hasler.de`; see root `AGENTS.md`'s Test Users note for why), but pointing either Environment at a genuinely separate host later is then just a secrets change, not a workflow restructuring. The rollback/`IMAGE_TAG` mechanics (#1733) and the `deploy-production`-only differences (no `DATABASE_SEED_ON_STARTUP` override, `ASPNETCORE_ENVIRONMENT=Production`) are unchanged from before the move - only which repo runs them changed.

**Required setup for the trigger to work at all:** a fine-grained PAT scoped to `mgmt-hetzner` with `Contents: write` (the minimum `repository_dispatch` needs), stored as this repo's `MGMT_HETZNER_DISPATCH_TOKEN` secret. Without it, `trigger-deploy` fails outright and no deploy happens for any tag, RC or stable.
...
2. Once `trigger-deploy` reports success, the actual deploy is now a separate workflow run in `maik-hasler/mgmt-hetzner` (`deploy-einsatzbereit.yaml`, fired by the dispatch) - poll its checks the same way, in that repo. Once it reports success, smoke-test live: `curl https://api.maik-hasler.de/health`, then HEAD-check `https://einsatzbereit.maik-hasler.de`.
3. If any publish job fails, diagnose from logs here. If `trigger-deploy` fails, `MGMT_HETZNER_DISPATCH_TOKEN` (this repo's secret) is the most likely cause. If mgmt-hetzner's `deploy-einsatzbereit.yaml` fails, its SSH/GHCR/app secrets (see that repo's `README.md`) are the most likely cause - this repo has no visibility into or control over that failure beyond re-firing the dispatch (`workflow_dispatch` on `deploy-einsatzbereit.yaml` also works for a manual retry without cutting a new tag).
