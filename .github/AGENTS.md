...
- **Jobs (all parallel, no `needs` between them):** `format-check`, `fast-tests` (Application.UnitTests + ArchitectureTests, scoped to just their own build graphs - no pnpm/Docker setup needed), `integration-tests` (IntegrationTests, needs pnpm + Docker pre-pull for the Aspire stack), `visual-tests` (VisualTests, same setup as `integration-tests`). All four jobs restore/build only their own project's dependency graph (`dotnet restore tests/<Project>` / `dotnet build tests/<Project> --no-restore`), never the whole `Einsatzbereit.slnx` - deliberately not shared via a build artifact across jobs (an upstream `needs: build` job would serialize what today runs in parallel, adding wall-clock time with no compile-error-detection benefit, since `IntegrationTests`/`VisualTests` each already transitively reference every `src/` project via `AppHost`/`Infrastructure`). Each then runs its test project(s) via `dotnet run --project ... --no-build`. Scoping `integration-tests`' build to just `tests/IntegrationTests` (instead of the whole solution) matters beyond speed: the whole-solution build used to also compile `VisualTests.csproj`, which downloads Chrome for Testing + Chrome Headless Shell (~290 MiB combined) from `cdn.playwright.dev` via its `InstallPlaywrightBrowsers` MSBuild target - wasted entirely, since `IntegrationTests` never touches Playwright. `visual-tests` additionally caches `~/.cache/ms-playwright` (`actions/cache`, keyed on `Directory.Packages.props`) so that download - now paid only once, where it's actually needed - doesn't repeat on every run either. Before this fix, that ~290 MiB CDN download happened twice per CI run (once wastefully in `integration-tests`, once for real in `visual-tests`); it's the actual source of the "gateway errors from downloading images repeatedly" symptom, not the three small Docker image pulls (`docker pull` in the "Pre-pull Docker images" step consistently completes in ~10-15s against real CI logs - not a measured source of flakiness).
