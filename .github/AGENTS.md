...
- **Jobs:**
  - `lint` (`pnpm lint` + `pnpm format:check` + `pnpm i18n:check` + the `check:nginx-*`/`check:config-defer`/`check:pwa-precache`/`check:pwa-manifest`/`check:pwa-offline-caching` scripts + `pnpm check` type check)
...
5. Tag with version, plus `latest` if the tag is not an RC. `publish-backend` still declares `outputs.version`/`outputs.prerelease`, but nothing consumes them - `github-release` re-derives the version from the tag itself
