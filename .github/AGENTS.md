...
├── publish.yml                 Tag-triggered: build + push backend/frontend/keycloak to GHCR, create a GitHub Release, then trigger a deploy in mgmt-hetzner - see "Publish Workflows" below
...
**Release candidates:** `-rc.N` suffix (e.g., `v1.0.0-rc.1`). Image is published, tagged `staging` instead of `latest`. Deploying it is a separate, manual step - see "Deployment moved to mgmt-hetzner" below.

**Full release:** Tag without `-rc` suffix → image published + `latest` tag updated. Same manual deploy step, targeting `production` instead.
...
5. Tag with version + `latest` (if not RC) or `staging` (if RC) - `publish-backend` also exposes this version string as a job output (`outputs.version`), consumed by `github-release` below. Nothing downstream of this consumes it to deploy - deploying is a manual step in a different repo, see below
...
### Deployment moved to mgmt-hetzner

Deployment (`docker-compose.yml`, the SSH/health-gate/rollback logic, and the Postgres/Keycloak/MinIO/SMTP/offsite-backup secrets it needs) does **not** live in this repo. It moved to `maik-hasler/mgmt-hetzner`, which also owns Traefik and the shared monitoring stack on the same host - see that repo's `README.md` and `.github/workflows/deploy-einsatzbereit.yaml`. `publish.yml` above stops once images are built and pushed and a GitHub Release is created; it has **no involvement at all** in what gets deployed where.

**Deliberately manual, not `repository_dispatch`.** An earlier version of this split had `publish.yml` fire a `repository_dispatch` event at mgmt-hetzner to trigger the deploy automatically, which needed a PAT (`MGMT_HETZNER_DISPATCH_TOKEN`) stored here with write access to that repo. That PAT was cut: the only thing that should cross the repo boundary for a routine release is a public GHCR image tag, not a credential. mgmt-hetzner's `deploy-einsatzbereit.yaml` is `workflow_dispatch`-only now - run by hand (the repo owner, or via the GitHub API by whoever is driving the release, e.g. a Claude Code session that already has access to both repos) with the version and target environment as inputs, once the tag's images are confirmed published here.

mgmt-hetzner's `deploy-einsatzbereit.yaml` mirrors the old `deploy-staging`/`deploy-production` split as two jobs/GitHub Environments (`einsatzbereit-staging`/`einsatzbereit-production`), selected by the `environment` input instead of a dispatched `prerelease` flag, for the same reason that split originally existed here (#1344): both currently deploy to the same single host and `/opt/einsatzbereit` directory (`einsatzbereit.maik-hasler.de`; see root `AGENTS.md`'s Test Users note for why), but pointing either Environment at a genuinely separate host later is then just a secrets change, not a workflow restructuring. The rollback/`IMAGE_TAG` mechanics (#1733) and the `deploy-production`-only differences (no `DATABASE_SEED_ON_STARTUP` override, `ASPNETCORE_ENVIRONMENT=Production`) are unchanged from before the move - only which repo runs them, and how they're triggered, changed.

**Required setup on the mgmt-hetzner side:** all the secrets `deploy-staging`/`deploy-production` used to read from this repo's `staging`/`production` GitHub Environments (`POSTGRES_*`, `KEYCLOAK_*`, `MINIO_*`, `SMTP_*`, `OFFSITE_S3_*`) need to be added to mgmt-hetzner's new `einsatzbereit-staging`/`einsatzbereit-production` Environments - GitHub never exposes existing secret values via API, so this is a manual copy in each repo's Settings -> Environments UI. mgmt-hetzner reuses its own existing `SSH_KEY`/`SSH_HOST`/`SSH_USER`/`CR_PAT` secrets to reach the host and pull images (it already has SSH access to this exact host for Traefik/monitoring/Blogfolio), plus a new `SSH_HOST_KEY` secret pinning that connection the same way this repo's old `STAGING_SSH_HOST_KEY` did. See mgmt-hetzner's `README.md` for the full secrets table. Nothing needs to be added here in this repo - there is no PAT to create.
...
`release-rc.yml` validates the branch suffix, promotes it to an annotated tag pushed with `RELEASE_TOKEN`, and deletes the branch - see the workflow's own top-of-file comment for the full mechanics. After the tag exists, `publish.yml` runs end-to-end (build -> GHCR -> dispatch), then mgmt-hetzner's `deploy-einsatzbereit.yaml` picks up the dispatch and deploys (`deploy-staging` job, for RC tags).
...
2. Deploying is manual - it will not happen on its own. Trigger mgmt-hetzner's `deploy-einsatzbereit.yaml` directly via `mcp__github__actions_run_trigger` (`method: run_workflow`, `owner: maik-hasler`, `repo: mgmt-hetzner`, `workflow_id: deploy-einsatzbereit.yaml`, `ref: main`, `inputs: {version: "<the version from step 1's tag, without the leading v>", environment: "staging"}` for an RC tag, `"production"` for a stable tag). This needs `mgmt-hetzner` to be added to the session's repo scope first if it isn't already.
3. Poll that workflow run's checks the same way, in `mgmt-hetzner`. Once it reports success, smoke-test live: `curl https://api.maik-hasler.de/health`, then HEAD-check `https://einsatzbereit.maik-hasler.de`.
4. If any publish job fails, diagnose from logs here. If mgmt-hetzner's `deploy-einsatzbereit.yaml` fails, its SSH/GHCR/app secrets (see that repo's `README.md`) are the most likely cause - this repo has no visibility into or control over that failure; re-run `deploy-einsatzbereit.yaml` (same inputs) once the underlying issue is fixed.

**Deploy secrets moved to mgmt-hetzner, with one deliberate exception.** `KEYCLOAK_BACKEND_SECRET`, `STAGING_SSH_HOST_KEY`, `MINIO_APP_ACCESS_KEY`/`MINIO_APP_SECRET_KEY`, `OFFSITE_S3_*`, and the rest of what `deploy-staging`/`deploy-production` used to read now belong to mgmt-hetzner's `einsatzbereit-staging`/`einsatzbereit-production` GitHub Environments - see that repo's `README.md` for the full table and for how each was originally captured (e.g. the SSH host key pinning rationale). `KEYCLOAK_BACKEND_SECRET`'s role (resolving the realm JSON's placeholder, matching the backend's `Keycloak__ClientSecret`) is still documented in `keycloak/AGENTS.md` - only where the value is stored moved. The exception: `reset-staging.yml` (see "Reset Workflow" below) was **not** moved along with the rest of deploy - it still SSHes into the host directly and still needs its own copy of these same secrets in this repo's `staging` GitHub Environment, so they are not fully gone from here. That duplication is a known follow-up, not an oversight - see that section for why.
...
**This workflow deliberately stayed in this repo when deployment moved to mgmt-hetzner.** It SSHes into the host directly (not via mgmt-hetzner) and needs its own copy of the `staging` GitHub Environment secrets to do so - it does not check out or depend on `docker-compose.yml` itself, only on that file already being deployed at `/opt/einsatzbereit` on the host, so the move didn't break it. That does mean a subset of secrets (`STAGING_SSH_KEY`/`STAGING_SSH_HOST`/`STAGING_SSH_USER`/`STAGING_SSH_HOST_KEY`, `POSTGRES_*`, `KEYCLOAK_*`, `MINIO_*`, `SMTP_*`, `OFFSITE_S3_*`) now has to be kept in sync in two places - this repo's `staging` Environment and mgmt-hetzner's `einsatzbereit-staging` Environment. Moving this workflow to mgmt-hetzner too (it would need no einsatzbereit-repo checkout at all) would remove that duplication - flagged as a follow-up, not done here since it wasn't part of what moved.

- Trigger: `workflow_dispatch` only, with a required `confirm` input that must equal exactly `RESET`
- Runs in the `staging` GitHub Environment, using its own `STAGING_SSH_*` secrets - including the pinned `STAGING_SSH_HOST_KEY` (#1733 - this is a destructive workflow, so it should be at least as strict about a DNS/BGP hijack of `STAGING_SSH_HOST` as a deploy job is, not less)
- Removes the `postgres_data` and `minio_data` volumes - `postgres_backups` and `minio_backups` are left alone on purpose, as the recovery path if a reset needs to be walked back
...
- Because Keycloak's realm import runs with `OVERWRITE_EXISTING` (see mgmt-hetzner's `apps/einsatzbereit/docker-compose.yml`) and the backend has `Database__MigrateOnStartup: true`, the stack re-migrates and re-imports `keycloak/realms/einsatzbereit-realm.json` on restart - the standard `vera`/`olaf`/`admin` test accounts come back automatically since they are defined in that checked-in realm config, not created ad hoc. This is by design: staging's demo credentials (including full-admin `admin`) are meant to be publicly known and instantly restorable, not rotated or hardened - see `keycloak/AGENTS.md` and #1166
